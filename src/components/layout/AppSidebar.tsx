...
import { Sidebar, SidebarContent, SidebarGroup, SidebarGroupContent, SidebarHeader, SidebarMenu, SidebarMenuItem, SidebarMenuButton, SidebarRail, SidebarSeparator, SidebarTrigger } from '@/components/ui/sidebar';
    ...
    '--sidebar-width-icon': '2.75rem' // Reduced icon width
      ...
      <SidebarHeader className="flex items-center justify-between">
        ...
        <div className="flex items-center px-2 py-3">
          <Link to="/app">
            {state === 'collapsed' ? <img src="/lovable-uploads/dfe9a235-bb0a-46f7-83e6-7b96aa0b49bd.png" alt="TRUSTY" className="h-7 w-auto object-contain" width={28} height={28} /> : <img src="/lovable-uploads/3fff3c36-d39f-4c4e-8e3f-56a242c5ac6f.png" alt="TRUSTY" className="h-7 w-auto" width={120} height={28} />}
          </Link>
        </div>
        <TooltipProvider>
          <Tooltip>
            ...
            <SidebarMenu>
              <SidebarMenuItem>
                <SidebarMenuButton asChild isActive={isActive('/app')}>
                  <Link to="/app" className={cn("text-gray-800 hover:bg-[rgb(247_247_247)]", isActive('/app') && "text-primary-blue bg-primary-blue/10", "text-base" // 16px font size
                )}>
                    <Home className={cn("h-5 w-5",
                  // Slightly larger icons to match text
              ...
              <SidebarMenuItem>
                <SidebarMenuButton asChild isActive={isActive('/app/links')}>
                  <Link to="/app/links" className={cn("text-gray-800 hover:bg-[rgb(247_247_247)]", isActive('/app/links') && "text-primary-blue bg-primary-blue/10", "text-base" // 16px font size
                )}>
                    <Link2 className={cn("h-5 w-5",
                  // Slightly larger icons to match text
            ...
            <SidebarMenu>
              <SidebarMenuItem>
                <SidebarMenuButton asChild isActive={isActive('/app/settings')}>
                  <Link to="/app/settings" className={cn("text-gray-800 hover:bg-[rgb(247_247_247)]", isActive('/app/settings') && "text-primary-blue bg-primary-blue/10", "text-base" // 16px font size
                )}>
